...
export default class slider_selector extends cc.Component {
    ...
    /* ***************引擎回调*************** */
    onLoad () {
        this._layout_o = this.node.getComponent(cc.Layout);
    ...
    private _check_touch_node(pos_o_: cc.Vec2): number {
        if (this._layout_o.horizontalDirection == cc.Layout.HorizontalDirection.LEFT_TO_RIGHT) {
            let temp1_n = Math.abs(this._layout_o.spacingX);
            let temp2_n = this.node.children[0].width + this._layout_o.spacingX;
            let temp3_n = 0;
            for (let i = this.node.children.length - 1; i >= 0; --i) {
                temp1_n += temp2_n;
                // 判断是否在节点范围内
                if (pos_o_.x > (this.node.width * 0.5 - temp1_n) && pos_o_.x < (this.node.width * 0.5 - temp3_n)) {
    ...
    private _touch_move(event_o_: cc.Touch): void {
        ...
        if (local_pos_o.x > -this.node.width * 0.5
            ...
            && local_pos_o.y > -this.node.height * 0.5
            && local_pos_o.y < this.node.height * 0.5)
        {
